...
# --- Configuration ---
BASE_SWEEP_OUTPUT_DIR = Path("testchr/eigensnp_sweeps_output").resolve()
METRICS_SCRIPT_PY_PATH = Path("testchr/metrics.py").resolve()
SAMPLE_FILE_METRICS = Path("testchr/igsr_samples.tsv").resolve()
PLOTS_OUTPUT_DIR = BASE_SWEEP_OUTPUT_DIR / "analysis_plots_v3_stacked_parallel"
...
def process_single_run_metrics_worker(task_args):
    ...
    # output_dir in sweeps_summary.tsv is the absolute path to the run's folder
    run_output_dir = Path(run_info["output_dir"]) 
    pca_file = run_output_dir / "eigensnp_results.eigensnp.pca.tsv"
    
    # Use a unique name for the metrics output for this specific call
            ...
            if metrics_output_temp_file.exists():
                ...
                for _, metrics_row in df_metrics_current_run.iterrows():
                    data_point = {
                        ...
                        "swept_param_value_str": str(run_info["swept_param_value"]),
                ...
                # os.remove(metrics_output_temp_file) # Clean up temp file
            else:
                # This case should ideally be caught by an exception in compute_metrics_for_superpopulations
                # Or metrics.py should sys.exit on failure.
                # print(f"Warning: Metrics script ran for {run_id} but output file {metrics_output_temp_file} not found.")
                pass # Error will be handled by empty run_metrics_results or lack of metrics
        except Exception as e:
            # print(f"Error calculating metrics for run {run_id} ({pca_file}): {e}")
            pass # Suppress error for now, will result in missing data for this run
    # else:
        # print(f"PCA file not found for run {run_id} at {pca_file}, skipping metrics.")
...
def main():
    ...
    # Identify runs that potentially have PCA files
    for _, run_info_series in df_sweep_summary.iterrows():
        run_info_dict = run_info_series.to_dict()
        # Check if PCA file is likely to exist (can refine this check)
        # The worker function will do the final check.
        tasks_for_pool.append((run_info_dict, str(BASE_SWEEP_OUTPUT_DIR)))

    print(f"Starting parallel metric calculation for {len(tasks_for_pool)} potential runs using {NUM_METRIC_CALC_WORKERS} workers...")
    ...
    unique_swept_eigensnp_params = sorted([
        ...
        if not pd.isna(p) and "Base Defaults" not in p
    ])
    ...
    for eigensnp_param_name in unique_swept_eigensnp_params:
        ...
        for metric_idx, metric_key in enumerate(METRIC_COLUMNS_TO_ANALYZE):
            ...
            df_plot_data_for_sweep[f"percent_better_{metric_key}"] = df_plot_data_for_sweep.apply(
                lambda row: calculate_percent_better(row.get(metric_key), row.get(f"baseline_{metric_key}"), metric_props["higher_is_better"]), axis=1
            )
            df_plot_ready = df_plot_data_for_sweep.dropna(subset=[f"percent_better_{metric_key}", "swept_param_value_numeric"])
            ...
            ax_right.plot(df_agg["swept_param_value_numeric"], df_agg["mean"], marker='s', linestyle='--', label="Mean")
            ax_right.plot(df_agg["swept_param_value_numeric"], df_agg["median"], marker='^', linestyle=':', label="Median")
            ax_right.set_title(f"{metric_props['name']}\nAggregated (Mean/Median)")
